...
pub(crate) static ADAPTER: GeminiMcpRegistration = GeminiMcpRegistration;

/// Gemini CLI's user-level MCP server configuration adapter.
pub struct GeminiMcpRegistration;

impl AgentMcpRegistrationAdapter for GeminiMcpRegistration {
    fn host_id(&self) -> &'static str {
        "gemini"
    }

    fn config_file(&self, host_home: &Path) -> PathBuf {
        host_home.join(".gemini").join("settings.json")
    }
    ...
    fn current_entry(
        ...
        let servers = servers.as_object().ok_or_else(|| {
            config_error("Gemini ~/.gemini/settings.json mcpServers must be an object")
        })?;
...
fn parse_config(config: Option<&[u8]>) -> Result<Map<String, Value>, AgentMcpRegistrationError> {
    ...
    let document: Value = serde_json::from_slice(config)
        .map_err(|_| config_error("Gemini ~/.gemini/settings.json is malformed JSON"))?;
    document
        .as_object()
        .cloned()
        .ok_or_else(|| config_error("Gemini ~/.gemini/settings.json must be a JSON object"))
}
...
fn serialize_config(document: Map<String, Value>) -> Result<Vec<u8>, AgentMcpRegistrationError> {
    serde_json::to_vec_pretty(&Value::Object(document))
        .map_err(|_| config_error("Gemini ~/.gemini/settings.json could not be serialized"))
}
