...
fn install_fake_gemini(directory: &Path) {
    #[cfg(unix)]
    {
        ...
        fs::write(
            &path,
            "#!/bin/sh\nprintf 'fake gemini response: %s\\n' \"$*\"\n",
        )
        .expect("fake Gemini executable should be writable");
        let mut permissions = fs::metadata(&path)
            .expect("fake Gemini executable should exist")
            .permissions();
        permissions.set_mode(0o755);
        fs::set_permissions(path, permissions)
            .expect("fake Gemini executable should be executable");
    }

    #[cfg(windows)]
    {
        fs::write(
            directory.join("gemini.cmd"),
            "@echo off\necho fake gemini response %*\n",
        )
        .expect("fake Gemini executable should be writable");
    }
}
...
fn start_gemini_fixture() -> (String, thread::JoinHandle<()>) {
