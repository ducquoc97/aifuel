...
pub(super) const RECOVERY_WINDOW: Duration = Duration::from_secs(5 * 60);
...
pub(super) fn parse_message(data: &[u8]) -> Result<ServerJsonRpcMessage, RemoteHttpError> {
...
pub(super) fn map_sse_error(error: SseReadError) -> RemoteHttpError {
    match error {
